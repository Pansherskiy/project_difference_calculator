from tests.fixtures.json_fixture import json_file1, json_file2, json_empty_file, empty_file # noqa
from difference_calculator.file_parsers.json_parser import json_generate_diff


def test_json_generate_diff(json_file1, json_file2, json_empty_file, empty_file):
  ...
  - verbose: true
    ...
    assert json_generate_diff(json_file1, json_empty_file) == '''{
  - follow: false
  - host: hexlet.io
  - proxy: 123.234.53.22
  - timeout: 50
}'''
    assert json_generate_diff(json_empty_file, json_file2) == '''{
  + host: hexlet.io
  + timeout: 20
  + verbose: true
}'''
